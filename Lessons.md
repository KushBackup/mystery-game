...
### 2026-08-21 — A restated fact drifts at the restatement, not at the source

**What happened:** Asked to deep-audit the Onam case for plot holes, I found twenty. Almost none were errors in [STORY.md](STORY.md); they were errors in the *copies*. The same 8:37 AM poison handoff is written in eleven places (STORY.md, `CASE_SOLUTION`, both reveal decks, three host docs, `hostReference.js`, PROJECT_CONTEXT) and one of them — Nikitha's flask sitting unopened under the beverage table all afternoon — silently contradicted the other ten, while being cited as evidence in a Round 1 accusation. The in-app reveal deck said "a third of the party" on slide 04 and "eleven people" on slide 13. Both decks titled the evidence ladder "Thirteen pieces of paper" over fourteen documents, having dropped the one clue that convicts Kalaivani on paper. Three witnesses heard sirens a minute before the ambulance was called.

**Why it was wrong:** The doc protocol in [Claude.md](Claude.md) says to update the relevant docs when a fact changes, and I have been following it — but "the relevant docs" is a judgement call made per edit, and each edit only touches the copies I happened to remember. The failure is structural rather than careless: eleven restatements of one fact will diverge unless something checks them *together*, and nothing in this repo does. The near-misses prove it from the other side — the eleven-person scatter is *exactly* eleven when you count the character timelines, and all 69 badge-ins fall inside the stated 8:20–9:40 AM window, because both of those were derived once and never re-typed as prose.

**What to do instead:** Audit the *set* of restatements, not the source. Three checks found most of this and cost nothing. **One:** extract every `timeline` string into one time-sorted table (331 beats here) and read it in order — that alone surfaced the sirens-before-the-call ordering, Akshat's discovery timestamped five minutes early, and the fact that Aarohi was the only one of 34 persons of interest with no gap inside the 2:45–3:25 PM window the incident report itself defines as the criterion for being one. **Two:** count every list-claim against its list ("thirteen pieces of paper" over fourteen rows; "eight forensic files" followed by seven names). **Three:** grep one distinctive noun — `flask`, `four ignored tickets`, `whole window` — across `src/`, `reveal-deck/` and `*.md` in a single pass, because a contradiction between two restatements is invisible from inside either one. Two mechanical traps when patching this much prose at once: `String.raw` does **not** interpret `\n`, so a newline meant for the *source* lands in the file as two literal characters; and an apostrophe written into a single-quoted JS string needs escaping. `npm run lint` catches both but reports only the first parse error per file, so re-run it until clean rather than once — and `npx esbuild <file> --outfile=/dev/null` is the faster loop for a data file.

### 2026-08-21 — A denormalised counter with no round key is wrong from the first round it survives

**What happened:** A fresh game opened on the ballot reporting **168 votes cast**. `submitVote` in [src/firebase/config.js](src/firebase/config.js) maintained a stored `voteCounts: { suspectId: count }` beside `votes: { userId: { round: suspectId } }`, and it decremented a player's *previous* pick only within the same round — nothing ever cleared it between rounds. So the figure the ballot printed was every vote the database had ever accepted, across all seven rounds and across the previous event, presented under a label the player reads as "the room, right now". The fix was to delete the stored counter and derive `voteCounts` from `votes` for `currentRound` in a `useMemo`.

**Why it was easy to miss:** The counter is correct on the surface it was written for — Round 1 of a clean database — and it degrades by a round at a time, so nothing ever *breaks*. `Numeral` even animates it convincingly. Both the ballot's stat block and the tally modal read the same wrong number, so they agreed with each other, which is what a bug looks like when the cache is the only source. And the label carried the round scope implicitly: "Votes cast" beside a per-round ballot means this round to a reader and all-time to the code.

**What to do instead:** When a document already holds the per-item records, don't store an aggregate beside them — derive it. If an aggregate must be stored, its key has to carry every dimension the display slices on (here: the round), or it is only correct until the first slice change. Two things that came out of the same fix and generalise: a stored aggregate is also a **read-then-replace** write, and with 69 phones on one ballot each voter overwrites whatever landed between their own `getDoc` and `setDoc` — `setDoc(ref, { votes: { [userId]: { [round]: pick } } }, { merge: true })` touches one nested path, loses nothing, and creates the document if it is missing. And retiring a field from the code leaves it in the live document forever unless a write removes it, so carry `deleteField()` on the next write rather than leaving a stale total for the next session to rediscover.

### 2026-08-21 — The security rule and the function that fights it are in the same repo and never read together

**What happened:** Players opened a fresh game and found the previous event's 265 chat messages still in Comms. `clearAllMessages()` in [src/firebase/config.js](src/firebase/config.js) looked correct — batch, delete every doc, commit — and Reset Game called it. But [firestore.rules](firestore.rules) said `allow update, delete: if false` on `messages/{messageId}`, so every commit was rejected, and the function's own `catch` logged to a console nobody was looking at and returned normally. The host had been resetting the game successfully for months and clearing nothing. Deleting the docs took one command through admin credentials (`firebase firestore:delete messages --recursive --force`), which bypasses rules entirely — the code fix was the smaller half.

**Why it was easy to miss:** Nothing about the JavaScript is wrong. The bug lives in the *gap* between two files that are never open at the same time: one grants a permission, the other spends it, and neither mentions the other. Worse, the rules file's own comment argued the opposite case convincingly ("nobody edits or deletes a message once sent" is a good rule for *players*) without noticing the host is not a player and has no separate identity to be granted — there is no Firebase Auth here, so "only the host may delete" is not expressible. And a swallowed error is exactly a silent success: `catch (e) { console.error(e) }` at the end of a mutating helper converts "the database refused" into "done".
