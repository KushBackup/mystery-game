...
- [src/hooks/](src/hooks/) — `useCountUp` (ticks a numeral on change, never on mount), `useTypewriter` (character-at-a-time reveal on one rAF loop) and `useUnreadMessages` (what the hub's **Comms** badge is carrying; the read watermark is a message id in `localStorage['astral.commsseen']`)
