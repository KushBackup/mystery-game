...
- [src/data/gameData.js](src/data/gameData.js) — 69 characters, 39 clue codes, the 12 statement pods, round definitions, case metadata, the riddle-lock reward queue, and host script (the largest data file)
...
- [src/components/RevealDeck.jsx](src/components/RevealDeck.jsx) — "How it happened": the **22-slide reconstruction**, opened from *both* terminal screens (the killer reveal and the outro) so all 69 players can reach it. Paged and swipeable, phone-first. It replaced the single scrolling `CaseSolution.jsx` document on 2026-08-07
...
- [src/components/views/](src/components/views/) — full-screen views (Dashboard, Intel, Chat, Voting, Timeline, Dossier, Help, Story). `IntelView` is the **Evidence** screen: one round-aware tabbed file. Case files are always present; Accusations, Motives, Evidence and Revelations appear only from their live rounds.
...
- [src/components/ui/](src/components/ui/) — the shared motion/vocabulary pieces: `Numeral` (counting brass figure), `RoundRail`, `RoundClock` (the per-round countdown), `RedactedLines`, `ScreenBrief`, `InfoTip` (the `?` tooltip), `FeedbackToast`, `SearchField` (the shared list filter — Vote and Guests), `Doodles`
- [src/hooks/](src/hooks/) — `useCountUp` (ticks a numeral on change, never on mount), `useTypewriter` (character-at-a-time reveal on one rAF loop), `useRoundClock` (ticks the round countdown, and only while one is running) and `useUnreadMessages` (what the hub's **Comms** badge is carrying; the read watermark is a message id in `localStorage['astral.commsseen']`)
- [src/lib/gameStart.js](src/lib/gameStart.js) — the **starting gun**: the three phases, the ten-second countdown, and the host's skip. Stored as an absolute instant for the same reason the round clock is — read the header before changing that
- [src/lib/tutorial.js](src/lib/tutorial.js) — the player arrival tutorial: per-player local stages, the next task's copy, and the hub destinations each stage exposes. It never writes Firestore state.
- [src/components/StandbyScreen.jsx](src/components/StandbyScreen.jsx) — the waiting screen every player sits on until the host presses **Start game**, and the countdown that lets the room in together
- [src/lib/roundTimer.js](src/lib/roundTimer.js) — the **round clock**: storage shape, the four states, and every transition as a pure function. Stored as an absolute end instant on the host's clock — read the header before changing that, it is the reason a phone that joins late lands on the right time
...
| Add/edit a character | [src/data/gameData.js](src/data/gameData.js) — characters array | Keep the roster at 69 unless the user explicitly changes event scale; a roster change must also rebalance `PODS` (the dev assertion will complain); update [STORY.md](STORY.md) if backstory changes |
...
| Add/rename/regroup an Evidence category | `CLUE_STACKS` in [src/data/gameData.js](src/data/gameData.js) (what it contains), `EVIDENCE_STACKS` in [src/data/screenGuide.js](src/data/screenGuide.js) (what it's called), `STACK_STYLE` in [IntelView.jsx](src/components/views/IntelView.jsx) (empty state) | Three files on purpose — data, copy, presentation. Tabs render only once their `opensAt` round is live, so new categories need an intentional opening round and concise tab label. |
...
| Start of game / the waiting screen | [src/lib/gameStart.js](src/lib/gameStart.js) for the model, [StandbyScreen.jsx](src/components/StandbyScreen.jsx) + [App.css](src/App.css) §13d for the screen, `startGate` in [App.jsx](src/App.jsx) for the gate, [HostPanel.jsx](src/components/HostPanel.jsx) for the controls | **Only the host writes a start**, and it is an absolute instant, not a flag — that is what stops a player who joins in Round 3 watching a starting gun fire. The gate is three states on purpose: `pending` is what keeps a late joiner from seeing a frame of a countdown they aren't owed. Host copy lives in four places and all four say the same thing: `HOST_SCRIPT` pregame in [gameData.js](src/data/gameData.js), [hostReference.js](src/data/hostReference.js), [HOST_LIVE_FACILITATION.md](HOST_LIVE_FACILITATION.md), [HOST_PRINT_PACK.md](HOST_PRINT_PACK.md) |
| Player arrival tutorial | [src/lib/tutorial.js](src/lib/tutorial.js) for stages/copy/access, [GridMenu.jsx](src/components/GridMenu.jsx) for the guided hub, [App.jsx](src/App.jsx) for completion | It is **local per player**, never shared Firestore state. Round 0 teaches identity, a guest profile, Comms and voting one destination at a time. Round 1 must always allow the Evidence lesson, even for an unfinished or late tutorial, so no player misses their accusation. |
| Round clock — length, states, urgency | [src/lib/roundTimer.js](src/lib/roundTimer.js) for the model, [RoundClock.jsx](src/components/ui/RoundClock.jsx) + [App.css](src/App.css) §13c for the display, [HostPanel.jsx](src/components/HostPanel.jsx) for the controls | **Only the host writes a timer** — 69 devices read the same end instant and none of them writes one, which is what stops them disagreeing. Expiry is never written either. Timer presets and start/reset remain available during ballot and results; confirmed previous/next round controls are the recovery override. Read [DESIGN_LANGUAGE.md](DESIGN_LANGUAGE.md) **§7.1** before touching urgency motion. |
| Story briefing copy or pacing | [src/data/storyIntro.js](src/data/storyIntro.js) only | It feeds both the fullscreen briefing and the Story screen. **Check any new line against the spoiler list in that file's header** — the poison/tumbler/camera-gap details are Round 3, the skim and Dev's real brief are Round 4, the conspiracy is Round 5+ |
