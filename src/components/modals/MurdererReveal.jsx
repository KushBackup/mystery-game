...
export const MurdererReveal = ({ currentRound, voteCounts, isRevealOpen }) => {
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-red-900/90 backdrop-blur-sm p-4">
      <div className="bg-red-600 text-white rounded-lg shadow-2xl border-8 border-red-800 w-full max-w-3xl p-8 text-center animate-pulse">
        ...
        <div className="mt-6">
          <button className="px-6 py-3 bg-white text-red-700 font-bold rounded shadow-lg border-2 border-red-800" onClick={(e)=>{e.stopPropagation(); /* noop: allow host to close from HostPanel */}}>OK</button>
        </div>
      </div>
      <style>{`
