...
Everyone who logs in lands on a waiting screen and stays there. Nothing in the app opens until you press Start game in the Game start section of this console, so let people arrive and log in at their own pace - there is nothing for them to wander into early.`,
    ...
    during: 'Help late arrivals log in - they will sit on the waiting screen until you start, and anyone who logs in after you start is simply let straight in with no countdown. Point everyone toward their Identity and Story screens before the first vote. The round clock starts with the game, so you do not need to touch the Round clock section unless you want a different length - 30 minutes by default, visible on every phone under the round number, and it never advances the round for you. ASK is not on their screens yet - it appears when you advance to Round 2 - so tell them the loop now and let the button arrive to a room that already knows what it is for. If anyone goes hunting for it early, that is the answer.',
    end: 'When the room is settled, press Start game. The room counts down from ten together, lands on the Round 0 briefing, and the clock starts as the countdown clears. If one phone is still showing the waiting screen after that, use Push start to everyone.',
  },
  {
    id: 0,
