...
**The unread badge** (`.er-badge`) is the smallest member of the family and the only one that is *positioned* rather than placed in flow — a count pinned to the top-right of a hub tile's icon (§9, Grid hub). It sits on aged paper, so the accent darkens to `signal-deep` with `bone` text, exactly as `.er-tag--onbone` does. 10px mono, `min-width: 19px`, square like everything else, `pointer-events: none` because the tile underneath is the button. Its one departure from the type rules: **tracking is `0.06em`, not `0.18em`.** Letter-spacing is applied after the last glyph, so at the system default a two-digit count sits visibly left of centre inside its own box — numbers are the one place the wide mono tracking works against the system. Measured at 1 and 2 digits, the glyphs sit 4.9px from each edge.

*Added 2026-08-07 for the Comms channel.* Never let a badge be the only channel: the tile's sub-label carries the same fact in words (`Encrypted` → `12 Unread`), which is what survives reduced motion and a player who simply wasn't looking (§7.1).
...
The per-round countdown, under the round numeral on the chrome rail and on the hub's masthead. Mono label, mono tabular digits, both on ink. Implemented as `.er-clock` (App.css §13c) / [`RoundClock`](src/components/ui/RoundClock.jsx).

**Mono, not brass.** Brass is for numerals only (§2.1), and the round figure one line above has already spent it — two brass numbers stacked in a 64px rail read as one two-line number. A clock is an instrument rather than a count, so it takes the chrome voice and spends its colour on *state* instead: `bone` for the body of the round, `signal-lift` for the last minute and for zero, `dim` while the host is holding it.

**An idle clock renders nothing.** A round the host chooses not to time looks exactly as it did before the clock existed, and no device shows a dead `30:00` while it waits for its first snapshot — a stopped clock is the one thing a countdown must never look like. The host's console is the one surface that draws an idle one, because over there the armed length is the thing being set.
...
**A cue whose trigger is unbounded gets the strictest budget of all, and changes axis rather than idea** (`.er-jog`, App.css §17). The third and last knock is the Comms tile's icon when the channel has something unread on it, and its trigger is the most frequent thing in the app — a message can land every few seconds once the room warms up, which is the entire point of the feature. So the budget is **three jogs per visit to the board, and the board is a fresh visit every time a screen closes.** Past that the badge (§6.1) and the sub-label carry it alone. Two notes on why this one is not simply `erSummon`. It **changes axis**: the other two knocks are a 14px vertical hop on a free-standing control, and this lives on a 32px icon centred in a fixed 4:3 paper tile with a badge pinned to its corner, where a 14px hop reads as the tile glitching rather than as the icon being pointed at. So it is a small lateral rock with a touch of tilt — the same idea, a different geometry. And it is deliberately **not `.er-shake`**, which is the same axis and would have been the obvious reuse: that one is the app's *refusal* (a rejected login code, a wrong riddle answer), and a new message is an invitation. ±4px over 550ms against `erShake`'s ±5px over 400ms, plus the tilt — near enough to read as a shake, far enough not to read as one.
